...
func NewLoggerWithService(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	// NOTE: The New Relic zerolog integration has been temporarily disabled here
	// because its upstream module layout is causing `go mod tidy` to fail.
	// If/when the upstream module is fixed you can reintroduce the integration.
	if cfg.IsProduction() && cfg.Logging.Format == "json" {
		// In production, write JSON logs to stdout
		writer = os.Stdout
	} else {
