...
func (s *TodoService) CreateTodo(ctx echo.Context, userID string, payload *todo.CreateTodoPayload) (*todo.Todo, error) {
	...
	if payload.ParentTodoID != nil {
		...
		if !parentTodo.CanHaveChildren() {
			err := errs.NewBadRequestError("parent todo can't have todos, (subtasks can't have subtasks)", false, nil, nil, nil)
			logger.Warn().Msg("parent todo can't have children")
			return nil, err
		}
	}
